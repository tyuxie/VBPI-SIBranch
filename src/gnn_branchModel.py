...
from src.gnnModels_slim import GNNStack, GatedGraphConv, IDConv, MeanStdPooling
...
class GNNModel(nn.Module):          
    ...
    def forward(self, tree_list, *args):
    ...
    def sample_branch(self, tree, batchsize):
        mean, std = self.mean_std(tree)
        samp_log_branch = torch.randn(batchsize, std.shape[0]) * std.exp() + mean - 2.0
