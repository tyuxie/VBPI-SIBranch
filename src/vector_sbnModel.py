...
from src.utils import BitArray
...
class SBN(nn.Module):
    ...
    def update_rootsplit_CPDs(self):
        ...
        if torch.isnan(self.rs_CPDs).any():
            raise Exception('Invalid rootsplit probability! Check self.rs:(max {:.4f}, min {:.4f})'.format(np.max(self.rs_CPDs.detach().numpy()), np.min(self.rs_CPDs.detach().numpy())))
